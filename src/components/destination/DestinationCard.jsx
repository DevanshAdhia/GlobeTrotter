...
const DestinationCard = ({ destination }) => {
  ...
  const ratingScore = destination.rating || 9.0;
  const ratingText = ratingScore >= 9.0 ? 'Exceptional' : ratingScore >= 8.5 ? 'Excellent' : 'Very Good';
  const reviewCount = destination.reviews || 309;
  const price = destination.startingPrice || 14500;
  const originalPrice = destination.originalPrice || Math.round(price * 1.15);
      ...
      <div className="p-4 flex flex-col justify-between flex-1 bg-white">
        <div>
          ...
          <p className="text-xs text-gray-500 font-normal mb-3 line-clamp-1">
            {destination.country || destination.subtitle || 'India'}
          </p>
