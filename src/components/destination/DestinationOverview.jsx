...
const DestinationOverview = ({ destination }) => {
  ...
  const isLong = destination.description.length > 250;

  return (
    <section className="mb-12">
      <h2 className="text-2xl font-bold text-gray-900 mb-4">About {destination.name}</h2>
      <div className="relative">
        <motion.div initial={false} animate={{ height: isExpanded || !isLong ? 'auto' : '100px' }} className="overflow-hidden text-gray-700 leading-relaxed text-lg relative">
